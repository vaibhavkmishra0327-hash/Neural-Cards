...
export function AdminPage() {
  ...
  const checkMissingTopics = async () => {
    try {
      ...
      if (nodes && existingTopics) {
        ...
        const uniqueMissing = missing.filter((v: any, i: any, a: any) => a.findIndex((t: any) => t.slug === v.slug) === i);
  ...
  const handleSaveToDB = async () => {
    if (contentType === 'flashcard' && generatedCards.length === 0) {
        setStatus('⚠️ No cards to save!'); return;
    }
    if (contentType === 'blog' && !generatedBlog) {
        setStatus('⚠️ No blog content to save!'); return;
    }
    ...
    const topicSlug = matchedPending 
        ? matchedPending.slug 
        : topic.toLowerCase().trim().replace(/ /g, '-').replace(/[^a-z0-9-]/g, '');
    ...
    if (contentType === 'flashcard') {
        ...
        if (!finalTopicId) {
            const { data: existing } = await supabaseClient.from('topics').select('id').eq('slug', topicSlug).single();
  ...
  return (
    <div className="container mx-auto px-4 py-12 max-w-4xl">
      ...
      <div className="bg-white dark:bg-zinc-900 p-6 rounded-xl border border-gray-200 dark:border-zinc-800 shadow-sm mb-8">
        <label htmlFor="topic-input" className="block text-sm font-medium mb-2 text-gray-700 dark:text-gray-300">
        ...
        <div className="flex gap-4">
            <input 
                ...
                placeholder={contentType === 'flashcard' ? "e.g. Transformers" : "e.g. A Deep Dive into Transformers"}
        ...
        {status && <p className="mt-4 text-sm font-medium text-purple-600 dark:text-purple-400 animate-pulse">{status}</p>}
      </div>
      ...
      {contentType === 'flashcard' && generatedCards.length > 0 && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            <div className="flex justify-between items-center mb-4">
                ...
                <button onClick={addCard} className="text-sm bg-gray-100 dark:bg-zinc-800 px-3 py-1 rounded-lg hover:bg-gray-200 flex items-center gap-1">
                ...
                {generatedCards.map((card, idx) => (
                    ...
                    >
                        ...
                        <div className="grid md:grid-cols-2 gap-4 mr-8">
                            <div>
                                <label htmlFor={`question-${idx}`} className="text-xs font-bold text-purple-600 uppercase mb-1 block">Question</label>
                                <textarea 
                            ...
                            <div>
                                <label htmlFor={`answer-${idx}`} className="text-xs font-bold text-green-600 uppercase mb-1 block">Answer</label>
                                <textarea 
      ...
      {contentType === 'blog' && generatedBlog && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mb-8">
              ...
              <label htmlFor="blog-content" className="sr-only">Blog Content</label> {/* 👈 Hidden Label */}
              <textarea 
