...
import { BookOpen, Flame, Trophy, Target, RefreshCw, ArrowRight, PlayCircle, Zap } from 'lucide-react';
  ...
  return (
    <div className="container mx-auto px-4 py-8 mt-16 text-foreground">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 gap-4">
        <div>
          <h1 className="text-3xl font-bold flex items-center gap-3">
            Welcome back, <span className="bg-gradient-to-r from-purple-600 to-pink-500 bg-clip-text text-transparent">{user?.email?.split('@')[0] || 'Learner'}</span>
        ...
        <button onClick={onSignOut} className="px-4 py-2 border border-gray-200 dark:border-zinc-700 rounded-lg hover:bg-gray-100 dark:hover:bg-zinc-800 transition-colors text-sm font-medium">
          Sign Out
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-12">
        {/* Streak */}
        <motion.div whileHover={{ y: -5 }} className="bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 p-5 rounded-2xl shadow-sm hover:shadow-md transition-all">
          <div className="flex items-center gap-4">
            <div className="p-3 bg-orange-100 dark:bg-orange-500/10 rounded-xl text-orange-600 dark:text-orange-500">
              <Flame className="w-6 h-6" />
            </div>
            <div>
              <h3 className="text-2xl font-bold">{displayStats.current_streak} <span className="text-sm font-medium text-muted-foreground">days</span></h3>
              <p className="text-xs font-bold text-orange-600 dark:text-orange-500 uppercase tracking-wider">Streak</p>
            </div>
          </div>
        </motion.div>
        ...
        <motion.div whileHover={{ y: -5 }} className="bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 p-5 rounded-2xl shadow-sm hover:shadow-md transition-all">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="text-2xl font-bold">{displayStats.cards_learned_total} <span className="text-sm font-medium text-muted-foreground">cards</span></h3>
              <p className="text-xs font-bold text-blue-600 dark:text-blue-500 uppercase tracking-wider">Learned</p>
            </div>
          </div>
        </motion.div>
        ...
        <motion.div whileHover={{ y: -5 }} className="bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 p-5 rounded-2xl shadow-sm hover:shadow-md transition-all">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="text-2xl font-bold">{displayStats.xp} <span className="text-sm font-medium text-muted-foreground">pts</span></h3>
              <p className="text-xs font-bold text-yellow-600 dark:text-yellow-500 uppercase tracking-wider">Total XP</p>
            </div>
          </div>
        </motion.div>

        {/* Daily Goal */}
        <motion.div whileHover={{ y: -5 }} className="bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 p-5 rounded-2xl shadow-sm hover:shadow-md transition-all relative overflow-hidden group">
          <div className="flex items-center gap-4 relative z-10">
            <div className="p-3 bg-purple-100 dark:bg-purple-500/10 rounded-xl text-purple-600 dark:text-purple-500">
              <Target className="w-6 h-6" />
            </div>
            <div>
              <h3 className="text-2xl font-bold">{displayStats.daily_cards_completed}<span className="text-muted-foreground text-lg">/{displayStats.daily_goal_target}</span></h3>
              <p className="text-xs font-bold text-purple-600 dark:text-purple-500 uppercase tracking-wider">Daily Goal</p>
            </div>
          </div>
          ...
          <div 
            ...
            style={{ width: `${Math.min((displayStats.daily_cards_completed / displayStats.daily_goal_target) * 100, 100)}%` }}
          />
        </motion.div>
      </div>
            ...
            {topics.map((topic, index) => (
              ...
              >
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400 line-clamp-2 mb-6">
                  {topic.description || "Master this topic with interactive flashcards and spaced repetition."}
                </p>
